...
def moving_correlation(first_data, second_data, col,no_of_days):
	...
	for i in range(len(list_1)):
		...
		if(i >= (no_of_days - 1)):
			...
			corr = numerator/denominator
			moving_cor.append(round(corr, 6))
...
def moving_correlation_2(list_1, list_2, no_of_days):
	...
	for i in range(len(list_1)):
		...
		xy = xy + (list_2[i] * list_1[i])
		x_sq_sum = x_sq_sum + (list_1[i] * list_1[i])
		y_sq_sum = y_sq_sum + (list_2[i] * list_2[i])

		if(i >= no_of_days - 1):

			x_mean = x_sum / no_of_days
			y_mean = y_sum / no_of_days
			...
			y_sum = y_sum - list_2[i - (no_of_days - 1)]
...
def Compare_2_stocks(first_stock,second_stock,s_date, e_date = '', mdays = 21, cdays = 21):
	...
	for i in range(len(moving_cor_1)):
		if(moving_cor_1[i] != moving_cor_2[i]):
			print(moving_cor_1[i], moving_cor_2[i])
			print("Not equal")
