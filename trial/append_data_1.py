...
def append_data(data, sym, e_date):
...
def ad_loop(sym, e_date):

	data = pd.read_csv('append_data_1.csv')
	data = data.set_index('Date')
	data.index = pd.to_datetime(data.index, format = '%Y-%m-%d %H:%M:%S')

	date = data.index[data.count()[sym]-1]

	l_date = data.index[::-1][0]
	e_date = datetime.strptime(e_date, "%Y-%m-%d")

	data = append_data(data, sym, l_date)
	data = append_data(data, sym, e_date)

	data.to_csv('append_data_1.csv')
...
ad_loop('AAPL', '2018-01-01')
ad_loop('XOM', '2018-01-01')
ad_loop('AMZN', '2018-01-01')
ad_loop('AMZN', '2019-01-01')
ad_loop('MRO', '2018-01-01')
ad_loop('CVX', '2018-01-01')
